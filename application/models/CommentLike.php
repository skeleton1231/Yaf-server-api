...
class CommentLikeModel extends PdoDb {
    ...
    public function __construct(){
        ...
        parent::__construct();
        self::$table = 'bibi_comments_likes';
    }
    ...
    public function getLike($feedId=0,$comment_id=0,$userId=0,$page=1){
        ...
        $sql = '
                ...
                FROM
                `bibi_comments_likes` AS t1
                LEFT JOIN
                `bibi_user_profile` AS t2
                ON
                t1.user_id = t2.user_id
                WHERE
                t1.feed_id = '.$feedId.' AND t1.comment_id ='.$comment_id.'
        ';
        if($userId){
     ...
     public function updateLikeNum($feedId,$commentId,$action='add'){
    ...
    public function deleteLike($feedId, $userId,$commentId){

        $key ='commentlike_'.$feedId.'_'.$userId.'_'.$commentId.'';

        $sql = 'DELETE FROM `bibi_comments_likes` WHERE `feed_id`='.$feedId.' AND `user_id`='.$userId.''.' AND `comment_id`='.$commentId.'';
