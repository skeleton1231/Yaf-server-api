...
class FavcarlikeModel extends PdoDb {
    ...
    public function deleteLike($CarId,$userId){

        $key = 'favoritecarlike_'.$CarId.'_'.$userId.'';

        $sql = 'DELETE FROM `bibi_fav_car_like` WHERE `car_id`='."'".$CarId."'".' AND `user_id`='.$userId.'';

        $rs = $this->execute($sql);

        if($rs){
            $sql='SELECT user_id,avatar,nickname FROM   `bibi_user_profile` WHERE user_id='.$userId.' ';
